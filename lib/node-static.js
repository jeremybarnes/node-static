...
this.Server.prototype.respond = function (pathname, status, _headers, files, stat, req, res, finish) {
    ...
    if (req.headers['if-none-match'] === headers['Etag'] &&
        ...
        if (this.cache && (key in exports.store) &&
            exports.store[key].stat.mtime >= stat.mtime && false) {
...
this.Server.prototype.stream = function (pathname, files, buffer, res, callback) {
    (function streamFile(files, offset) {
         //sys.debug("files = " + sys.inspect(files));
        ...
        if (file) {
            ...
            if (true) {
                ...
                fs.writeSync(res.socket.fd, res._header, null, 'binary');
                res.socket._headerSent = true;

                function finished(err, written)
                {
                    //sys.debug("written = " + written);
                    fs.closeSync(fd);
                    ...
                    //sys.debug("streamFile");
                }

                fs.sendfile(res.socket.fd, fd, offset, 1000000, finished);

                //sys.puts("sendfile fd=" + fd + " offset=" + offset + " written " + written);
            }
            else {
